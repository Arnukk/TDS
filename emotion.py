...
class Emotion:
    ...
    def get_children(self, childset=[]):
        """
        Returns the array of all the children of the emotion.
        @param childset: empty array
        @return childset
        """
        for child in self.children:
            childset.append(child.name)
            if child.nb_children() > 0:
                Emotion.get_children(child, childset)
        return childset
